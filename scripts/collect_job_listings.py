...
def collect_job_listings(config):
    ...
    def run(playwright):
        ...
        for job_board in config["job_boards"]:
            ...
            page.wait_for_selector(f'.{job_board["listing_class"]}', timeout=10000)

            job_elements = page.query_selector_all(f'.{job_board["listing_class"]}')

            for job_element in job_elements:
                try:
                    title = job_element.query_selector(
                        f'.{job_board["title_tag"]}'
                    ).inner_text()
                    company = job_element.query_selector(
                        f'.{job_board["company_class"]}'
                    ).inner_text()
                    location = job_element.query_selector(
                        f'.{job_board["location_class"]}'
                    ).inner_text()
                    description = job_element.query_selector(
                        f'.{job_board["description_class"]}'
                    ).inner_text()
                    jobs.append(
                        {
                            "title": title,
                    ...
                    print(
                        f"Failed to extract job details from {job_board['name']}: {e}"
                    )
                    continue
            browser.close()
